...
class Boid:
    ...
    def _clampSpeed(self) -> None:
        if self.speed>config.BOID_SPEED_MAX:
            self.speed = config.BOID_SPEED_MAX
    ...
    def _screen_wrap(self) -> None:
        ...
        elif self.x + config.BOID_SIZE >= config.WIDTH:
            self.x = 0+config.BOID_SIZE

        if self.y <= 0:
            self.y = config.HEIGHT-config.BOID_SIZE
        elif self.y + config.BOID_SIZE >= config.HEIGHT:
            self.y = 0+config.BOID_SIZE
